...
import com.ahmedmourad.mirror.compiler.MirrorCommandLineProcessor
...
internal object MirrorImportHandler {

    /**
     * The path to the IntelliJ compatible compiler plugin version.
     */
    private val PLUGIN_JPS_JAR: String?
        get() = PathManager.getJarPathForClass(MirrorCommandLineProcessor::class.java)

    /**
     * The [Logger] instance for this class.
     */
    private val logger = Logger.getInstance(MirrorImportHandler::class.java)
    ...
    private fun validateJar(path: String): Boolean {
        return try {
            ...
            manifest.mainAttributes.getValue("Implementation-Title")
                    .startsWith("com.ahmedmourad.mirror.gradle")
        } catch (_: Exception) {
            false
        }
    }
